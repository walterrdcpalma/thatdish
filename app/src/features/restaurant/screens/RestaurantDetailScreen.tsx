...
import {
  ...
  getMockDishes,
  getRestaurantSignature,
} from "@/src/features/dish/services";
...
export function RestaurantDetailScreen() {
  ...
  const allDishes = getMockDishes();
  const signature = restaurant
    ? getRestaurantSignature(allDishes, restaurant.id)
    : undefined;
  const restaurantDishes = restaurant
    ? allDishes.filter((d) => d.restaurantId === restaurant.id).slice(0, MAX_RESTAURANT_PHOTOS)
    : [];
